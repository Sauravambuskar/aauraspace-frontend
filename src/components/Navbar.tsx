    ...
    <>
      ...
      >
        <div className="mx-auto flex max-w-[1400px] items-center justify-between px-5 py-4 md:px-10 md:py-5">
          ...
          {/* Desktop nav */}
          <nav className={`hidden items-center gap-8 text-sm font-medium md:flex lg:gap-10 text-ink/70`}>
            {NAV_LINKS.map((l) => (
        ...
        {mobileOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-y-0 right-0 z-[60] flex w-[85vw] max-w-sm flex-col bg-white shadow-2xl md:hidden"
            >
              ...
              <div className="flex items-center justify-between px-7 py-6">
                ...
                <button onClick={() => setMobileOpen(false)} aria-label="Close menu" className="grid h-9 w-9 place-items-center rounded-full border border-ink/20 text-ink hover:border-copper hover:text-copper transition-colors">
                  ✕
                </button>
              </div>

              {/* Divider */}
              <div className="mx-7 h-px bg-copper/15" />
                ...
                {NAV_LINKS.map((l, i) => (
                  ...
                  >
                    <Link
                      to={l.to}
                      onClick={() => setMobileOpen(false)}
                      className="group flex items-baseline justify-between border-b border-ink/8 py-5"
                    >
                      <span className={`font-serif text-4xl transition-colors ${location.pathname === l.to ? "text-copper" : "text-ink group-hover:text-copper"}`}>
                        {l.label}
                      </span>
                      <span className="text-xs uppercase tracking-[0.18em] text-ink/30 group-hover:text-copper transition-colors">
                        {l.num}
                      </span>
                    </Link>
                  </motion.div>
              ...
              >
                ...
                {/* Quick contact */}
                <div className="mt-6 space-y-2 text-center text-xs text-ink/40">
